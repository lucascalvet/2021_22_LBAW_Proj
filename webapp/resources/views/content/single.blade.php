...
<div class="container-fluid pb-3">
  ...
  <div class="text-center text-secondary">
    ...
    <br>
    In response to <a class="text-truncate" href="{{ route('content.show', ['id' => $content->contentable->parent->first()->id_content]) }}">{{ $content->contentable->parent->first()->content->creator->name }}</a>
    @endif
  </div>
  <div>
    ...
    <div class="d-flex justify-content-center mt-3 mb-3">
      ...
      <form method="POST" action="{{ $link_remove }}">
        @csrf
        @method('PATCH')
        <button type="submit" class="btn btn-outline-warning btn-lg text-dark me-3">Remove
          from Group</button>
      </form>
      @endif
        ...
        <ul class="list-group">
          ...
          <li class="list-group-item">
            <div class="d-flex flex-row align-items-center">
              ...
              @if (Auth::check() && $comment->author == $user)
              <form method="POST" action="{{ route('comment.destroy', $comment->id) }}">
                @csrf
                @method('DELETE')
                <button class="btn btn-outline-danger ms-3" type="submit"><i class="bi bi-trash"></i></button>
              </form>
              @endif
          ...
          <li class="list-group-item">
            <form method="POST" action="{{ route('content.comment', ['id' => $content->id]) }}">
              ...
              <div class="d-flex flex-row">
                <input type="text" id="comment-text" name='comment_text' class="form-control" aria-describedby="comment-help-block">
        ...
        <ul class="list-group">
          ...
          <li class="list-group-item">
            <div class="d-flex flex-row align-items-center">
              ...
              <span class="text-end text-nowrap">
                ...
                <a class="text-dark ms-1" href="{{ route('content.show', ['id' => $reply->content->id, '#comments']) }}">
          ...
          <li class="list-group-item">
            <form method="POST" action="{{ route('textcontent.create') }}">
              ...
              <div class="d-flex flex-row">
                <input type="text" id="comment-text" name='post_text' class="form-control" aria-describedby="comment-help-block">
