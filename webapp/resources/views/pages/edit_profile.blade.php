...
<section id="editProfile">
  ...
  <div class="container pb-3">
    ...
    <div class="row gx-5">
      <form class="input-group" action="{{ route('profile.save', ['user' => $user->id]) }}" method="POST">
        @csrf
        @method('PATCH')
        <div class="col-6">
          <label for="name" class="form-label ps-3 mt-2 mb-0 fw-bold">Name</label>
          <input id="name" name="name" type="text" class="form-control" value="{{$user->name}}" aria-label="Name" />

          <label for="email" class="form-label ps-3 mt-2 mb-0 fw-bold">Email</label>
          <input id="email" name="email" type="email" class="form-control" value="{{$user->email}}" aria-label="Email" />

          <label for="phone" class="form-label ps-3 mt-2 mb-0 fw-bold">Phone Number</label>
          <input id="phone" name="phone" type="text" class="form-control" value="{{$user->phone_number}}" aria-label="Phone Number" />

          <label for="birthday" class="form-label ps-3 mt-2 mb-0 fw-bold">Birthday</label>
          <input id="birthday" name="birthday" type="date" class="form-control" value="{{$user->birthday}}" aria-label="Birthday" />

          <label for="password" class="form-label ps-3 mt-2 mb-0 fw-bold">Password</label>
          <input id="password" name="password" type="password" class="form-control" placeholder="Keep empty to mantain the current password" aria-label="Password" />

          <label for="description" class="form-label ps-3 mt-2 mb-0 fw-bold">Description</label>
          <input id="description" name="description" type="text" class="form-control pb-5" value="{{$user->description}}" aria-label="Description" />
        ...
        <div class="col-6 text-center ps-3">
          ...
          <div class="d-flex justify-content-center">
            <div class="bg-white rounded-circle p-5" style="max-width: 7em; max-height: 7em;">
              <img id="img-profile-picture" style="display: none;">
              <!--FIXME: the image uploaded overloads the circle div-->
          ...
          <div class="bg-white rounded-3 p-5">
            <!--FIXME: do the same thing as the profile picture input-->
